...
ZunResult MidiOutput::Play()
{
    ...
    this->LoadTracks();
    this->midiOutDev.OpenDevice(g_Supervisor.cfg.midiDeviceId);
...
ZunResult MidiOutput::StopPlayback()
{
    ...
    else
    {
        ...
        // Send Note Off for every note that the sequencer thinks is currently playing.
        // This is more reliable than CC 123 because it targets exactly the notes
        // that are stuck, using the same mechanism as normal playback.
        for (i32 ch = 0; ch < 16; ch++)
        {
            for (i32 note = 0; note < 128; note++)
            {
                u32 byteIdx = note / 8;
                u32 bitIdx = note % 8;
                if (this->channels[ch].keyPressedFlags[byteIdx] & (1 << bitIdx))
                {
                    this->midiOutDev.SendShortMsg(0x80 | ch, note, 0);
                    this->channels[ch].keyPressedFlags[byteIdx] &= ~(1 << bitIdx);
                }
            }
            // Also send CC 123 + 120 as a fail-safe
            this->midiOutDev.SendShortMsg(0xB0 | ch, 123, 0);
            this->midiOutDev.SendShortMsg(0xB0 | ch, 120, 0);
        }
...
void MidiOutput::OnTimerElapsed()
{
    ...
    if (!trackLoaded)
    {
        // Send All Notes Off before looping, to prevent stuck notes
...
void MidiOutput::ProcessMsg(MidiTrack *track)
{
    ...
    case MIDI_OPCODE_MODE_CHANGE:
        ...
        case 2:
            ...
            break;
        case 4:
            // Foot controller — loop jump.
            // Before jumping, send All Notes Off on all channels to
            // prevent stuck notes from the "skipped" section of the track.
            utils::DebugPrint2("MIDI: loop jump triggered, sending All Notes Off\n");
            for (i32 ch = 0; ch < 16; ch++)
            {
                bool ok = this->midiOutDev.SendShortMsg(0xB0 | ch, 123, 0);
                if (!ok)
                    utils::DebugPrint2("MIDI: SendShortMsg CC123 ch%d FAILED\n", ch);
            }
            // Now perform the loop jump
            for (i32 i = 0; i < this->numTracks; i++)
            {

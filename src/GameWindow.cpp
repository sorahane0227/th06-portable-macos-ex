...
u32 g_ViewportOffsetX = 0;
u32 g_ViewportHeight = GAME_WINDOW_HEIGHT;
u32 g_ViewportOffsetY = 0;
f32 g_WidthResolutionScale = 1.0f;
f32 g_HeightResolutionScale = 1.0f;

void UpdateViewportDimensions()
{
    // Compute viewport to preserve 4:3 aspect ratio with letterboxing/pillarboxing
    if (g_GameWindowWidthReal * 3 > g_GameWindowHeightReal * 4)
    {
        // Window is wider than 4:3 — pillarbox horizontally
        g_ViewportWidth = (u32)((g_GameWindowHeightReal / 3.0f) * 4.0f);
        g_ViewportOffsetX = (g_GameWindowWidthReal - (i32)g_ViewportWidth) / 2;
        g_ViewportHeight = (u32)g_GameWindowHeightReal;
        g_ViewportOffsetY = 0;
    }
    else if (g_GameWindowWidthReal * 3 < g_GameWindowHeightReal * 4)
    {
        // Window is taller than 4:3 — letterbox vertically
        g_ViewportWidth = (u32)g_GameWindowWidthReal;
        g_ViewportOffsetX = 0;
        g_ViewportHeight = (u32)((g_GameWindowWidthReal / 4.0f) * 3.0f);
        g_ViewportOffsetY = (g_GameWindowHeightReal - (i32)g_ViewportHeight) / 2;
    }
    else
    {
        // Perfect 4:3
...
void GameWindow::CreateGameWindow()
{
    ...
    // Query the actual window size (may differ from defaults if using fullscreen desktop)
    if (g_SdlWindow != NULL)
    {
        SDL_GetWindowSize(g_SdlWindow, &g_GameWindowWidthReal, &g_GameWindowHeightReal);
